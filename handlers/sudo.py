...
def sudo(client, message,redis):
	...
	if text and (type is "private" or (type is "supergroup" or type is "group")) :
		if rank == "sudo":
			if re.search("^رفع نسخه الاحتياطيه$|^رفع نسخة الاحتياطيه$", text):
				msgID = Bot("sendMessage",{"chat_id":chatID,"text":"انتظر قليلاً جاري تحميل الملف ℹ️","reply_to_message_id":message.message_id,"parse_mode":"html","disable_web_page_preview":True})["result"]["message_id"]
			...
			if re.search("^نسخه الاحتياطيه للمجموعات$|^ رفع نسخة الاحتياطية للمجموعات$", text):
			...
			if re.search("^تغير الاساسي$|^تغير الاساسي @(.*)$|^تغير الاساسي [0-9]+$", text):
				if re.search("@",text):
					user = text.split("@")[1]
				if re.search("^تغير الاساسي [0-9]+$" ,text):
					user = text.split(" ")[2]
				if message.reply_to_message:
					user = message.reply_to_message.from_user.id
				if 'user' not in locals():return False
				try:
					...
					Bot("sendMessage",{"chat_id":chatID,"text":f"✅꒐ تم تغير المطور الاساسي الى {userFn} {userId}","reply_to_message_id":message.message_id,"parse_mode":"html"})
				except Exception as e:
					print(e)
					Bot("sendMessage",{"chat_id":chatID,"text":r.userNocc,"reply_to_message_id":message.message_id,"parse_mode":"html"})
